		...
		for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
			...
			*/
			if (result.kind === "server") {
				pool.penalize(account.id, void 0, modelId);
				logger?.warn(`dsh-workbuddy-xdpool: ${account.label} saw upstream ${result.status} (attempt ${attempt + 1}/${maxAttempts}); rotating`);
				continue;
			}
...
async function poolWebStatus(deps, region = "cn") {
	...
	const now = Date.now();
	/**
	* [xdpool-usage] Consumption ledger for this poll.
	*
	* The read is cached process-wide for a minute because the card polls this
	* route far more often than a human reads it, and the file only changes when
	* those very polls observe new consumption. The first poll after startup
	* records nothing at all: every account is a first observation, so the ledger
	* is seeded from the balances that exist right now and no historic backlog is
	* mistaken for today's usage.
	*/
	...
	const ledger = usageLedgerCache.ledger;
	...
	for (const account of accounts) {
		...
		if (!row.cooling) {
			try {
				...
				*/
				if (row.credits !== void 0 && Number.isFinite(row.credits.total)) {
					remainingTotal = finiteOr(remainingTotal) + row.credits.total;
					try {
						if (noteUsageObservation(ledger, account.id, row.credits.total, now)) usageDirty = true;
					} catch {}
				}
			} catch (error) {
...
*/
const WORKBUDDY_POOL_SETTINGS_NS = "llm-workbuddy-xdpool";
